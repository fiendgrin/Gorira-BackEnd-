...
function inputCreator(tagToPass) {
    return `<input for="TagNames" name="TagNames" class="singleTag" type="text" hidden value="${tagToPass
        .toLowerCase()
        .trim()}"/>`;
}
...
function tagCreator(tagToPass) {
  ...
  ${tagToPass.toLowerCase().trim()}
    <img class="close" id=${tagToPass
            .toLowerCase()
            .trim()
            .replaceAll(" ", "#")} src="/assets/images/icons/plus.svg"/>
  </div>`;
}
...
numberInputs.forEach((numberInput) => {
    numberInput.addEventListener("input", function () {
        ...
        if (decimalCount > 1) {
            ...
            inputValue = splitValues[0] + "." + splitValues.slice(1).join("");
        }
